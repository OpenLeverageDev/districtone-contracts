...
import {ISpaceShareV2} from "./ISpaceShareV2.sol";
import {IErrors} from "./IErrors.sol";
import {Erc20Utils, IERC20} from "../common/Erc20Utils.sol";
import {ReentrancyGuard} from "../common/ReentrancyGuard.sol";
import {SignatureLibV2} from "../libraries/SignatureLibV2.sol";
import {BlastAdapter} from "../BlastAdapter.sol";

/**
 * @title SpaceShareV2.sol Contract
 ...
 * Implements ISpaceShareV2.sol and IErrors interfaces.
 */
contract SpaceShareV2 is BlastAdapter, IErrors, ReentrancyGuard, ISpaceShareV2 {
    using Erc20Utils for IERC20;
    using SignatureLibV2 for SignatureLibV2.SignedData;
    ...
    function buyShares(uint256 spaceId, uint256 shares, uint256 maxInAmount, uint256 timestamp, bytes memory signature) external override {
        SignatureLibV2.SignedData memory signedData = SignatureLibV2.SignedData(_msgSender(), timestamp, spaceId, shares, true);
    ...
    function buySharesTo(uint256 spaceId, uint256 shares, uint256 maxInAmount, uint256 timestamp, bytes memory signature, address to) external override {
        SignatureLibV2.SignedData memory signedData = SignatureLibV2.SignedData(to, timestamp, spaceId, shares, true);
        if (!signedData.verify(signature, signIssuerAddress, signValidDuration)) revert InvalidSignature();
    ...
    function sellSharesV2(uint256 spaceId, uint256 shares, uint256 minOutAmount, uint256 timestamp, bytes memory signature) external override {
        SignatureLibV2.SignedData memory signedData = SignatureLibV2.SignedData(_msgSender(), timestamp, spaceId, shares, false);
        if (!signedData.verify(signature, signIssuerAddress, signValidDuration)) revert InvalidSignature();
